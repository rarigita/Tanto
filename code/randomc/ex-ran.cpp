...
int main() {
  ...
  // or:
  // TRanrotWGenerator mg(seed);       // make instance of random number generator
  // or:
  TRandomMotherOfAll cg(seed);      // make instance of random number generator
  ...
  printf("\n\nRandom integers mersene in interval from 0 to 1:\n");
  for (i=0; i<70; i++) {
    ir = rg.IRandom(0,1);
    printf ("%i  ", ir);}
  ...
  // make random integers in interval from 0 to 99, inclusive:
     printf("\n\nRandom integers mother in interval from 0 to 1:\n");
     for (i=0; i<70; i++) {
        ir = cg.IRandom(0,1);
        printf ("%i  ", ir);}
